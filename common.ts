...
import { readableStreamToString, RottenTomatoesError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new RottenTomatoesError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new RottenTomatoesError(e, e.response?.data)
                }
                if (innerError instanceof RottenTomatoesError) {
                    // Got "RottenTomatoesError" from the above try block
                    throw innerError;
                }
